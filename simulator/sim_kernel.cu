...
__device__
inline float4 matDecode(char encoding, const float4 *__restrict__ materials, char matCount) {
	char mask = 0x01;
...
__global__
inline void integrateBodies(float4 *__restrict__ newPos, float4 *__restrict__ newVel,
