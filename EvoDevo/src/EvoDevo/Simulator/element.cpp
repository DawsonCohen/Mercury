...
namespace EvoDevo {
    ...
    */
    ...
    void Element::GroupFaces() {
        ...
        while(markCount < faces.size()) {
            ...
            for(uint i = 0; i < faces.size(); i++) {
                ...
                if(massMarks[f.m0] || massMarks[f.m1] || massMarks[f.m2]) continue;
    ...
    void Element::GroupSprings() {
        ...
        std::vector<bool> massMarks(masses.size());
        std::vector<bool> springMarks(springs.size(), false);
        uint16_t markCount = 0;
        ...
        while(markCount < springs.size()) {
            ...
            for(uint i = 0; i < springs.size(); i++) {
                ...
                if(massMarks[s.m0] || massMarks[s.m1]) continue;
            ...
            springGroups.push_back(std::vector<Spring>(group));
        }
    }
