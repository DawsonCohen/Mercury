...
namespace EvoDevo {
	...
	void Simulator::Simulate(float sim_duration, bool trackStresses, bool trace, std::string tracefile) {
		...
		#ifdef EV_DEBUG
			static std::unordered_map<std::string, int> trace_hist;
			if(!trace_hist[tracefile]) trace_hist[tracefile] = 0;

			FileWriter fileWriter(tracefile, trace_hist[tracefile]);
			fileWriter << "id, time, x, y, z, vx, vy, vz\n";
		#endif
			...
			if(trace) {
				if(step_count % 20 == 0) {
					...
					for(unsigned int i = 0; i < numMasses; i++) {
						...
						fileWriter << i << "," << m_total_time << ","; 
						fileWriter << pos.x << "," << pos.y << "," << pos.z << ",";
						fileWriter << vel.x << "," << vel.y << "," << pos.z << "\n";
					}
				}
			}
			#endif
	...
	void key_value_sort(float* d_keys_in, float* d_keys_out, uint* d_values_in, uint* d_values_out, uint items_per_segment, uint num_segments) {
		...
		int* d_offsets;
		cudaMalloc((void**) &d_offsets, (num_segments+1) * sizeof(int));
