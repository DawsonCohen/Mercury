...
namespace EvoDevo {
    ...
    void Optimizer<T>::WriteSolutions(const std::vector<T>& solutions, const std::string& directory) {
        ...
        Util::WriteFile(config.io.base_dir + "/latest.txt",directory);

        float fitness;
        std::string encoding, filename, filepath;
    ...
    std::vector<T> Optimizer<T>::NoNicheSolve() {
        ...
        while(Evaluator<T>::eval_count < max_evals) {
            ...
            fitness_history_writer << generation << "," << Evaluator<T>::eval_count << "," << archived_solution.fitness() << "\n";
            ...
            if(generation % config.optimizer.save_skip == 0 || eval_count >= max_evals) {
                ...
                population_history_writer << generation << "," << eval_count << ",";
                diversity_history_writer << generation << "," << eval_count << ",";

                for(i = 0; i < population.size(); i++) {
                    population_history_writer << population[i].fitness();
                    // diversity_history_writer << diversity[i];
                }
                population_history_writer << "\n";
                diversity_history_writer << "\n";
            }
            generation++;
        }
    ...
    std::vector<T> Optimizer<T>::Solve(Config config) {
        ...
        for(int N = 0; N < opt_config.repeats; N++) {
            ...
            std::string fitness_filepath(working_directory+"/fitness_history.csv");
            std::string population_fitness_filepath(working_directory+"/population_history.csv");
            std::string population_diversity_filepath(working_directory+"/diversity_history.csv");
            fitness_history_writer.Open(fitness_filepath);
            population_history_writer.Open(population_fitness_filepath);
            diversity_history_writer.Open(population_diversity_filepath);

            fitness_history_writer << "generation, evaluation, solution_fitness\n";
            population_history_writer << "generation, evaluation, "; 
            diversity_history_writer << "generation, evaluation, "; 
            for(uint i = 0; i < population.size(); i++) {
                population_history_writer << "organsim_" << i; 
                diversity_history_writer << "organsim_" << i; 
            }
